...
create index if not exists ix_prediction_outcomes_horizon_eval on public.prediction_outcomes(horizon_minutes, evaluated_at desc);
...
create index if not exists ix_paper_risk_decisions_prediction on public.paper_risk_decisions(prediction_id, created_at desc);
...
create index if not exists ix_monitoring_symbol_created on public.model_monitoring_windows(symbol, created_at desc);
...
create index if not exists ix_retraining_status_requested on public.retraining_requests(status, requested_at);
...
create index if not exists ix_closed_loop_runs_type_started on public.closed_loop_runs(run_type, started_at desc);
...
create index if not exists ix_breaker_created on public.circuit_breaker_events(breaker, created_at desc);
...
set config = jsonb_set(config, '{live_enabled}', 'false'::jsonb, true), updated_at = now()
where name='global';
...
as $$ select public.invoke_edge_function('execute-paper'); $$;
...
returns bigint language sql security definer set search_path = public, pg_temp
as $$ select public.invoke_edge_function('evaluate-closed-loop'); $$;
...
create or replace function public.closed_loop_health(p_symbol text default 'XAUUSD_PROXY_GC') returns jsonb
language sql stable security definer set search_path = public, pg_temp
as $$
with latest_ingestion as (
  select status, watermark, finished_at, details from public.data_ingestion_runs order by started_at desc limit 1
), latest_prediction as (
  select id, action, probability, model_version, risk_state, created_at from public.model_predictions where symbol=p_symbol order by created_at desc limit 1
), latest_outcome as (
  select prediction_id, horizon_minutes, realized_return, signed_return, action_correct, evaluated_at from public.prediction_outcomes where symbol=p_symbol order by evaluated_at desc limit 1
), latest_monitor as (
  select model_version, observations, brier, accuracy, coverage, trade_win_rate, mean_signed_return, drift_score, data_age_minutes, status, created_at from public.model_monitoring_windows where symbol=p_symbol order by created_at desc limit 1
), retraining as (
  select count(*) filter(where status='pending')::int as pending, max(requested_at) as latest_request_at, max(finished_at) filter(where status='succeeded') as latest_success_at from public.retraining_requests where symbol=p_symbol
), paper as (
  select count(*) filter(where status='filled')::int as open_orders, count(*) filter(where status='closed')::int as closed_orders from public.paper_orders where symbol=p_symbol
), breaker as (
  select state, reason, created_at from public.circuit_breaker_events order by created_at desc limit 1
), live_flag as (
  select coalesce((config->>'live_enabled')::boolean,false) as live_enabled from public.risk_limits where name='global'
)
