...
type LoopHealth = {
  ...
  liveEnabled?: boolean;
  monitoring?: { status?: string; observations?: number; brier?: number; trade_win_rate?: number; mean_signed_return?: number; drift_score?: number; data_age_minutes?: number };
  retraining?: { pending?: number; latest_request_at?: string; latest_success_at?: string };
  paper?: { open_orders?: number; closed_orders?: number };
  circuitBreaker?: { state?: string; reason?: string; created_at?: string };
  outcome?: { horizon_minutes?: number; signed_return?: number; action_correct?: boolean; evaluated_at?: string };
};
type Snapshot = { mode: "paper" | "live" | "demo"; symbol: string; price: number; changePct: number; modelProbability: number; action: "BUY" | "SELL" | "NO_TRADE"; riskState: string; modelVersion: string; bars: Bar[]; closedLoop?: LoopHealth };
...
export default function GoldTerminal() {
  ...
  const loop = data.closedLoop ?? {};
  const monitor = loop.monitoring ?? {};
  const barDenominator = Math.max(data.bars.length - 1, 1);

  return <main className="shell">
    <header className="topbar"><div className="brand"><div className="brandMark"/><div><h1>Gold Decision Lab</h1><p>Data → model → backtest → paper → outcome → retraining</p></div></div><div className="status"><span className={`pill ${data.mode === "live" ? "live" : ""}`}>{data.mode.toUpperCase()}</span><span className="pill">{data.modelVersion}</span></div></header>
    <section className="gridKpi">
      ...
      <div className="card kpi"><label>Decision</label><strong className={actionClass}>{data.action}</strong><small>NO_TRADE is a first-class decision</small></div>
      <div className="card kpi"><label>Model confidence</label><strong>{(data.modelProbability * 100).toFixed(1)}%</strong><small>Probability of an upward 15m move</small></div>
      <div className="card kpi"><label>Monitoring</label><strong style={{fontSize:17}}>{monitor.status ?? data.riskState}</strong><small>{monitor.observations ?? 0} labeled observations</small></div>
      <div className="card kpi"><label>Execution</label><strong style={{fontSize:17}}>{(loop.executionMode ?? "paper").toUpperCase()}</strong><small>Live enabled: {loop.liveEnabled ? "YES" : "NO"}</small></div>
    </section>
    <section className="layout">
      <div className="card chartCard">
        ...
        <svg className="oscSvg" viewBox="0 0 1000 150" preserveAspectRatio="none"><line x1="0" y1="75" x2="1000" y2="75" stroke="#262b27"/>{data.bars.map((b,i)=>{const x=(i/Math.max(data.bars.length,1))*1000,w=1000/Math.max(data.bars.length,1)-2,h=Math.min(Math.abs(b.squeeze)*2.2,60);return <rect key={i} x={x} y={b.squeeze>=0?75-h:75} width={Math.max(w,.5)} height={h} fill={b.squeeze>=0?"#39d98a":"#ff5c66"} opacity=".58"/>})}<path d={wt1} transform="translate(0 15)" fill="none" stroke="#4fb2ff" strokeWidth="2" vectorEffect="non-scaling-stroke"/><path d={wt2} transform="translate(0 15)" fill="none" stroke="#cf625d" strokeWidth="2" vectorEffect="non-scaling-stroke"/><path d={rsi} transform="translate(0 15)" fill="none" stroke="#bb79ff" strokeWidth="1.5" opacity=".8" vectorEffect="non-scaling-stroke"/></svg>
      </div>
      <aside className="side">
        <div className="card signalTicket"><div style={{color:"var(--muted)",fontSize:11,textTransform:"uppercase",letterSpacing:'.1em'}}>Next action ticket</div><div className={`signalSide ${actionClass}`}>{data.action}</div><div className="signalMeta"><div className="metric"><span>P(up 15m)</span><b>{(data.modelProbability*100).toFixed(1)}%</b></div><div className="metric"><span>Risk gate</span><b>{data.riskState}</b></div></div><div className="warn">The system can abstain. Paper decisions are evaluated after costs; live broker execution remains disabled.</div></div>
        <div className="card"><b>Closed-loop health</b><div className="timeline" style={{marginTop:14}}><div className="row"><span>Prediction outcomes</span><b>{loop.outcome?.horizon_minutes ? `${loop.outcome.horizon_minutes}m labeled` : "warming up"}</b></div><div className="row"><span>Brier score</span><b>{monitor.brier == null ? "—" : Number(monitor.brier).toFixed(3)}</b></div><div className="row"><span>Paper trades</span><b>{loop.paper?.closed_orders ?? 0} closed</b></div><div className="row"><span>Retraining queue</span><b>{loop.retraining?.pending ?? 0} pending</b></div><div className="row"><span>Circuit breaker</span><b>{loop.circuitBreaker?.state ?? "CLOSED"}</b></div></div></div>
        <div className="card"><b>Decision stack</b><div className="timeline" style={{marginTop:14}}><div className="row"><span>1. UT / trend candidate</span><b>gate</b></div><div className="row"><span>2. WaveTrend + RSI + Squeeze</span><b>features</b></div><div className="row"><span>3. ML meta-label</span><b>probability</b></div><div className="row"><span>4. Risk + circuit breaker</span><b>veto</b></div><div className="row"><span>5. Paper execution</span><b>outcome</b></div><div className="row"><span>6. Monitoring</span><b>drift</b></div><div className="row"><span>7. Challenger retrain</span><b>promotion</b></div></div></div>
